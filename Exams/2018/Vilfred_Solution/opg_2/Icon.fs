...
let rec eval (e : expr) (cont : cont) (econt : econt) = 
    ...
    | Find(pat, str) -> 
        let rec find (i:int) =
            ...
            if index <> -1 
            then 
                cont (Int index) (fun _ -> find (index+1))
            else
                econt ()
        find 0  
    | Fail -> econt ()
...
// 1
let iconEx1 = Every(Write(And(FromTo(1, 2), FromTo(3,4))))

// 2 
let iconEx2 = Every(Write(CstS "icon"))
(* 
    iconEx2 writes each Right for each Left.
    The nested 'And' creates a newline after FromTo(1,4)
    So each nested 'And' will be created for each of the first FromTo(1,4)

*)

// 3 
let str1 = "Hi there - if there are anyone"
let str2 = "this this is is stu stuttering"
let str3 = "hey you, yea you right there, no you!"
