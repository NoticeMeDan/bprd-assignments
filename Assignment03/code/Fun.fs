...
type value = 
  | Int of int
  | Closure of string * string list * expr * value env       (* (f, x, fBody, fDeclEnv) *)   // Exercise 4.3
...
let rec eval (e : expr) (env : value env) : int =
    ...
    | Call(Var f, eArgs) -> 
      ...
      | Closure (f, x, fBody, fDeclEnv) ->
        let xVals = eArgs |> List.map(fun value -> Int(eval value env)) |> List.zip x    // Exercise 4.3
        let fBodyEnv = xVals @ (f, fClosure) :: fDeclEnv                                // Exercise 4.3
        eval fBody fBodyEnv
